# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 11:29:51 2020
...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 12:11:22 2020
...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 13:57:14 2020
...
def get_semantic_related(word,n=30):
    ...
    #print(words)  
    return words 
def get_semantic_related1(word,n=30):
    words=[]
    if(word in word_vectors.vocab):
        pb=word_vectors.most_similar(word,topn=30)
        for i,p in enumerate(pb):
...
def get_syntactic_related(word,n=30):
        words=[]
...
#    synonyms=[]
    try:
        synonyms=find_closest_embeddings(embeddings_dict[word])[:50]
    except:
        synonyms=[]        
    ...
    def __init__(self,path,task,data,step,replacements=20):
        ...
        if(step=='train'):
            ...
            if (not os.path.exists(path+"vectorizer.pickle")):
                vectorizer=CountVectorizer(tokenizer=generate_tokens,min_df=mindf, max_df=maxdf)
                mapping = vectorizer.fit_transform(self.corpus['text'])
                dmad.close()
        ...
        self.DEFINEDACTIONS=['get_synonyms','get_semantic_related','get_semantic_related1','bitsquatting', 'insertion','omission','addition','generatehom','num_to_word','currency_to_word','word_to_num','word_weekday','word_month']
        ...
        self.get_important_words()
        self.original_state=list(self.importantwords.keys())
...
#        return self.Word_Action_Mapping
    ...
    def update_state(self,wordtochange,newtoken):
        ...
        index=self.original_state.index(wordtochange)
...
#                             globalchange=round((previous_score-new_score)/previous_score,3)
##                             localchange=change
#                             rewardofadversarialexamples=extrareward
#                             reward=localchange+rewardofadversarialexamples)/3)
...
file1 = open(path+model+"_logging_final.txt","a+") 
file1.close()
seedfile=path+'trainseed.csv' # Path of the Corpus
###Called only onces
logfile=path+'AnalysisLog_'+task+model
log=open(logfile, 'a+' ,encoding='utf-8',newline='')
logwriter= csv.writer(log)
...
log.close()
AdversarialFile=path+'Adversarial'+task+model
adv=open(AdversarialFile, 'a+' ,encoding='utf-8',newline='')
ad= csv.writer(adv)
